...
# O-HAZE 的 hazy / GT 路径（按你自己的目录结构来）
HAZY_DIR = Path("data/raw/ohaze/hazy")
GT_DIR   = Path("data/raw/ohaze/gt")

# 输出去雾结果
OUTPUT_DIR = Path("data/results/clahe_full_ohaze")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 结果指标保存成 CSV
METRICS_CSV = OUTPUT_DIR / "metrics_clahe_ohaze.csv"

# 统一缩放的长边像素（和 iHAZE 保持一致）
LONG_SIDE = 720   # 或 512 / 1024，看你算力

# CLAHE 参数（可以和之前 iHAZE 那边保持一致）
...
def main():
    ...
    rows = []
    print(f"【O-HAZE】共发现 {len(hazy_files)} 张 hazy 图，开始处理...")

    for hazy_path in hazy_files:
        fname = hazy_path.name

        # ⚠ 这里假设命名规律和 iHAZE 类似：
        # 如：xxx_hazy.jpg → xxx_GT.jpg
